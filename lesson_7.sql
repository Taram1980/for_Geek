...
SELECT name, birthday_at, users.id FROM users JOIN orders ON users.id = orders.user_id;
SELECT * FROM orders;
SELECT * FROM users;

SELECT name FROM users WHERE users IN (SELECT user_id FROM orders);
...
SELECT f.id, c.name, c2.name FROM cities c JOIN flights f ON c.label = f.`from` JOIN cities c2 ON f.`to` = c2.label ORDER BY f.id;
